...
*     SP: added XR parameter
      SUBROUTINE LMBMU(N,NA,MCU,MC,NW,X,XR,F,RPAR,IPAR,IOUT,TIME,RTIM,W)
...
*     Array Arguments
      INTEGER IPAR(*),IOUT(*)
*     SP: added XR
      DOUBLE PRECISION X(*),XR(1000,*),RPAR(*),W(*)
...
*     SP: added XR
     ...
     &     TMPMC5(*),TMPMC6(*),TMPN1(*),TMPN2(*),TMPMAT(*),XR(1000,*)
...
* SP: store the current iterate
      IF (NIT .LT. 1000) THEN
*        CALL COPY(N, X, XR(1, NIT))
        DO 11 I = 1,N
          XR(NIT + 1,I) = X(I)
  11    CONTINUE
      END IF
