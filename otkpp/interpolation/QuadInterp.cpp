...
QuadInterp::QuadInterp(const Function &f, const vector< double > &xb, double delta)
{
  int i;
...
double QuadInterp::eval(const vector< double > &d)
{
  return c_ + inner_prod(g_, d) + 0.5*inner_prod(d, prod(H_, d));
}

double QuadInterp::evalLagrangian(int j, const vector< double > &d)
{
...
const vector< double > &QuadInterp::getG() const
{
  return g_;
}

const vector< double > &QuadInterp::getGl(int i) const
{
  return gl_[i];
}
...
void QuadInterp::initialize_(const vector< double > &xb, double delta)
{
  double D;
...
void QuadInterp::computeLagrangeCoeff_first_(int i, const vector< double > &xb)
{
  if(i > 0)
  {
    double coeff;
...
void QuadInterp::computeLagrangeCoeff_last_(const vector< double > &xb, double denom, int i, int j, int k)
{
...
void QuadInterp::computeLagrangeCoeff_hat_(const vector< double > &xb, double denom1, double denom2, int j)
{
